...
# Create a proper Automator workflow for EisenVault context menu

echo "Creating EisenVault context menu using Automator..."

# Create a temporary AppleScript file
TEMP_SCRIPT="/tmp/eisenvault_upload.scpt"
...
on run {input, parameters}
    ...
    repeat with filePath in input
        set fileList to fileList & (POSIX path of filePath)
    end repeat
    
    set filePathsString to ""
    repeat with i from 1 to count of fileList
        set filePathsString to filePathsString & item i of fileList
        if i < count of fileList then
            set filePathsString to filePathsString & ","
        end if
    end repeat
    
    do shell script "open -a 'EisenVault Desktop' --args 'eisenvault://upload?files=' & quoted form of filePathsString"
    
    return input
end run
EOF

# Create the Automator workflow using osascript
WORKFLOW_PATH="$HOME/Library/Services/Upload to EisenVault.workflow"

# Create the workflow directory
mkdir -p "$WORKFLOW_PATH/Contents"

# Create a minimal but valid Info.plist
...
# Create a simple workflow document
...
echo "Context menu created successfully!"
